...
f0 = dp.get_freq_max_power(psd, freq_axis)
print('lo frequency: ', f0)
...
class SDRWorker(QObject):
    ...
    def run(self):
        ...
        np.append(all_samples, samples)
        print(len(all_samples))
        np.save(file, all_samples)

        psd = 10*np.log10(np.abs(np.fft.fftshift(np.fft.fft(samples)))**2)
        freq_axis = np.linspace(-sample_rate/2, sample_rate/2, len(psd))
        freq_max_power = dp.get_freq_max_power(psd, freq_axis)
        beat_freq = dp.compute_beat_freq(freq_max_power, f0)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.imageitem.setLevels((-80, 20))
